...
void TmxRenderer::Render(CIwFVec2 offset)
{
	int numLayers = map->layers.size();
	for(int l=0; l<numLayers; ++l)
		RenderLayer(l, offset, CIwFVec2(0,0));
}

void TmxRenderer::Render(CIwFVec2 offset, CIwFVec2 destCellSize)
{
	int numLayers = map->layers.size();
	for(int l=0; l<numLayers; ++l)
...
bool TmxRenderer::isTileVisible(CIwFVec2 destination)
{
	return  ( -displayTileSize.x < destination.x) && (destination.x< displayWidth) &&
			( -displayTileSize.y < destination.y) && (destination.y< displayHeight);
}

void TmxRenderer::RenderLayer(int layerIdx, CIwFVec2 offset, CIwFVec2 destCellSize)
{
